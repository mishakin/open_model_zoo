...
int main(int argc, char *argv[]) {
    try {
        ...
        // Create plugins for devices
        std::vector<std::pair<std::string, std::string>> cmdOptions = {
            {FLAGS_d, FLAGS_m}, {FLAGS_d_fd, FLAGS_m_fd},
            {FLAGS_d_hp, FLAGS_m_hp}, {FLAGS_d_lm, FLAGS_m_lm}
        };

        std::map<std::string, InferencePlugin> pluginsPool;
        createPluginsPool(cmdOptions, pluginsPool, FLAGS_pc);

        // Set up face detector and estimators
        FaceDetector faceDetector(FLAGS_m_fd, pluginsPool[FLAGS_d_fd], FLAGS_t, FLAGS_fd_reshape);

        HeadPoseEstimator headPoseEstimator(FLAGS_m_hp, pluginsPool[FLAGS_d_hp]);
        LandmarksEstimator landmarksEstimator(FLAGS_m_lm, pluginsPool[FLAGS_d_lm]);
        GazeEstimator gazeEstimator(FLAGS_m, pluginsPool[FLAGS_d]);
